...
- 公共社区只展示已通过审核的动态。
- “我的作品”展示本人所有未删除作品，包括已发布、已发布待复核和审核未通过。
...
6. 审核不通过或已删除的作品不能点赞；机器标记待复核的作品仍可正常互动。
...
2. 无评论时展示“来聊聊这个话题吧～”。
3. 评论支持文字与最多 3 张图片混发，也支持纯文字或纯图片；使用键盘发送后立即出现在列表中。
4. 社区列表和详情页评论数同步增加，重新进入后数量保持一致。
5. `community_comments` 保存评论者 ID、昵称、头像、地区、正文、COS 图片及异步审核状态；未设置地区时不展示。
6. 违规文本不写入数据库；图片先公开并异步审核，微信返回 `risky` 后整条评论自动隐藏并扣减数量。
...
14. 图片评论和图片回复均可点击预览；微信返回 `pass/review` 时继续展示。
...
8. 公共列表只隐藏人工审核明确设为 `rejected` 的内容；机器 `review/risky` 仍正常展示。

### 我的作品

1. 展示本人所有 `status=active` 的作品，不按审核状态过滤。
2. 头像、昵称、图片和定位与社区卡片一致。
3. 状态分别显示：已发布、已发布待复核、审核未通过。
4. 图片审核回调未完成时页面可按需刷新，但不影响作品公开展示。
...
- 正文和定位文字调用微信 `msgSecCheck` V2；`pass/review/risky` 均允许发布，未知结果或接口异常阻止发布。
- 带图片的动态把 COS 公网图片地址提交给微信 `mediaCheckAsync`，数据库创建时直接写入 `reviewStatus=approved` 并立即公开。
- 机器 `pass` 保持 `adminReviewStatus=not_required`；`review/risky` 保持公开，同时写入 `adminReviewStatus=pending` 进入后台待复核。
- 只有后台人工选择“不通过”才写入 `reviewStatus=rejected` 并停止公开展示；机器结果不会直接下架作品，也不产生机器审核通知。
- 公共列表只展示 `approved`。

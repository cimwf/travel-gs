...
> 本文保留社区首次上线的部署和排障记录，部分审核、举报、评论图片和权限逻辑已经
> 变化。当前上线请使用 [统一上线检查清单](release-checklist.md)，架构判断请使用
> [当前架构与业务规则](current-architecture.md)。
...
默认由管理员审核通过。新建官方动态固定写入 `dataEnv=dev`，只供开发版和体验版
确认内容；确认无误后在官方动态列表点击“发布”，云函数才会改为 `dataEnv=prod`，
并把 `createdAt` 更新为实际发布时刻。编辑内容不能直接修改发布环境。
